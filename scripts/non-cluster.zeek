...
event DeepStats::process_epoch_result(ss: SumStat, now: time, data: ResultTable)
	{
	# TODO: is this the right processing group size?
	local i = 50;
	local keys_to_delete: vector of DeepStats::Key = vector();
	...
	if ( |data| > 0 )
		...
		schedule 0.01 secs { DeepStats::process_epoch_result(ss, now, data) };
	...
	if ( ss$name in result_store )
		...
		if ( ss?$epoch_result )
			...
			if ( bro_is_terminating() )
				...
				{
				event DeepStats::process_epoch_result(ss, now, copy(data));
				}
			}
