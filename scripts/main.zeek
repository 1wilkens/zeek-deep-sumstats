...
export {
	...
	type Reducer: record {
		...
		pred:           function(key: DeepStats::Key, obs: DeepStats::Observation): bool &optional;

		## A function to normalize the key.  This can be used to
		## aggregate or normalize the entire key.
		normalize_key:  function(key: DeepStats::Key): Key &optional;
	};
	...
	type SumStat: record {
		...
		## to calculate a value from the :zeek:see:`DeepStats::Result` structure
		## which will be used for thresholding.
		## This function is required if a *threshold* value or
		## a *threshold_series* is given.
		threshold_val:      function(key: DeepStats::Key, result: DeepStats::Result): double &optional;
		...
		threshold_crossed:  function(key: DeepStats::Key, result: DeepStats::Result) &optional;

		## A callback that receives each of the results at the
		## end of the analysis epoch.  The function will be
		## called once for each key.
		epoch_result:       function(ts: time, key: DeepStats::Key, result: DeepStats::Result) &optional;
        ...
        ## Group that this SumStat belongs to
	...
	global create: function(ss: DeepStats::SumStat);
	...
	global observe: function(id: string, key: DeepStats::Key, obs: DeepStats::Observation);

	## Dynamically request a sumstat key.  This function should be
	## used sparingly and not as a replacement for the callbacks
	## from the :zeek:see:`DeepStats::SumStat` record.  The function is only
	...
	## Helper function to represent a :zeek:type:`DeepStats::Key` value as
	## a simple string.
	##
	## key: The metric key that is to be converted into a string.
	##
	## Returns: A string representation of the metric key.
	global key2str: function(key: DeepStats::Key): string;
}
