##! This implements transparent cluster support for the DeepStats framework.
...
event zeek_init() &priority=100
	{
	Broker::auto_publish(Cluster::manager_topic, DeepStats::cluster_send_result);
	Broker::auto_publish(Cluster::manager_topic, DeepStats::cluster_key_intermediate_response);
	Broker::auto_publish(Cluster::manager_topic, DeepStats::send_a_key);
	Broker::auto_publish(Cluster::manager_topic, DeepStats::send_no_key);
	}
...
function data_added(ss: SumStat, key: Key, result: Result)
	...
	if ( check_thresholds(ss, key, result, cluster_request_global_view_percent) )
		...
		event DeepStats::cluster_key_intermediate_response(ss$name, key);
		add recent_global_view_keys[ss$name, key];
		}
	}

event DeepStats::get_a_key(uid: string, ss_name: string, cleanup: bool)
	{
	if ( uid in sending_results )
		{
		if ( |sending_results[uid]| == 0 )
			{
			event DeepStats::send_no_key(uid, ss_name);
			}
		else
			{
			for ( key in sending_results[uid] )
				{
				event DeepStats::send_a_key(uid, ss_name, key);
...
event DeepStats::cluster_ss_request(uid: string, ss_name: string, cleanup: bool)
	{
...
event DeepStats::cluster_get_result(uid: string, ss_name: string, key: Key, cleanup: bool)
	{
	...
	if ( cleanup ) # data will implicitly be in sending_results (i know this isn't great)
		...
		if ( uid in sending_results && key in sending_results[uid] )
			...
			event DeepStats::cluster_send_result(uid, ss_name, key, copy(sending_results[uid][key]), cleanup);
			...
			event DeepStats::cluster_send_result(uid, ss_name, key, table(), cleanup);
			}
		}
	else
		{
		if ( ss_name in result_store && key in result_store[ss_name] )
			...
			# Note: copy is needed to compensate serialization caching issue. This should be
			# changed to something else later.
			event DeepStats::cluster_send_result(uid, ss_name, key, copy(result_store[ss_name][key]), cleanup);
			}
		else
			{
			# We need to send an empty response if we don't have the data so that the manager
...
event DeepStats::cluster_threshold_crossed(ss_name: string, key: DeepStats::Key, thold_index: count)
	{
...
event zeek_init() &priority=100
	{
	Broker::auto_publish(Cluster::worker_topic, DeepStats::cluster_ss_request);
	Broker::auto_publish(Cluster::worker_topic, DeepStats::cluster_get_result);
	Broker::auto_publish(Cluster::worker_topic, DeepStats::cluster_threshold_crossed);
	Broker::auto_publish(Cluster::worker_topic, DeepStats::get_a_key);
	}
...
event DeepStats::finish_epoch(ss: SumStat)
	...
	if ( network_time() > zero_time )
		...
		event DeepStats::cluster_ss_request(uid, ss$name, T);
...
function data_added(ss: SumStat, key: Key, result: Result)
	...
	if ( check_thresholds(ss, key, result, 1.0) )
		...
		event DeepStats::cluster_threshold_crossed(ss$name, key, threshold_tracker[ss$name][key]);
		}
	}
...
function handle_end_of_result_collection(uid: string, ss_name: string, key: Key, cleanup: bool)
	...
	if ( check_thresholds(ss, key, ir, 1.0) )
		...
		event DeepStats::cluster_threshold_crossed(ss_name, key, threshold_tracker[ss_name][key]);
		}
...
function request_all_current_keys(uid: string, ss_name: string, cleanup: bool)
	...
	if ( uid in stats_keys && |stats_keys[uid]| > 0 )
		...
		for ( key in stats_keys[uid] )
			{
			done_with[uid] = 0;
			event DeepStats::cluster_get_result(uid, ss_name, key, cleanup);
		...
		done_with[uid] = 0;
		#print fmt("get_key by uid: %s", uid);
		event DeepStats::get_a_key(uid, ss_name, cleanup);
		}
	}

event DeepStats::send_no_key(uid: string, ss_name: string)
	{
	#print "send_no_key";
...
event DeepStats::send_a_key(uid: string, ss_name: string, key: Key)
	{
...
event DeepStats::cluster_send_result(uid: string, ss_name: string, key: Key, result: Result, cleanup: bool)
	{
...
event DeepStats::cluster_key_intermediate_response(ss_name: string, key: Key)
	{
	...
	event DeepStats::cluster_get_result(uid, ss_name, key, F);
	}
